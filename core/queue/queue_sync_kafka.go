...
func (k KafkaSyncQueue) Publish(ctx context.Context, message *queuemessage.Message) error {
	messageBytes, err := k.opts.MessageProcessor.Encode(message)
	if err != nil {
		return err
	}
	topic := k.opts.Topic
	k.opts.Logger.Debugf(ctx, "publish topic:%s,message:%s", topic, messageBytes)
...
func (k KafkaSyncQueue) Consume(ctx context.Context, integration interface{}) (err error) {
	...
	concreteMsg, err := k.opts.MessageProcessor.Decode(consumerSessionMessage.Message.Value)
	if err != nil {
		return err
	}
