...
消息 trace 用于定位单条消息在 ws 集群内的链路耗时。入口按 `trace_id` 做确定性采样，采样命中的消息会透传 `trace` 上下文；未采样消息不额外携带 trace，避免影响高频推送性能。慢请求、失败、队列满等异常会按限流生成诊断 trace。trace 不在消息体内累加事件，关键节点会输出结构化日志，通过 `trace_id` 在日志系统中聚合同一条链路。
